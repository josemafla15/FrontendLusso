...
import type { DestinoContenido, Paginado } from "@/lib/types";

export default function DestinosPage() {
  const [destinos, setDestinos] = useState<DestinoContenido[]>([]);
  ...
  const load = useCallback(async (q: string) => {
    ...
    try {
      ...
      setDestinos(Array.isArray(res) ? res : res.results);
    } catch (e) {
  ...
  return (
    <div>
      <div className="flex flex-wrap items-end justify-between gap-3">
        <h1 className="font-display text-3xl font-semibold">Destinos</h1>
        <Link
          href="/destinos/nuevo"
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {destinos.map((d) => (
            ...
            >
              ...
              <p className="flex-1 font-semibold">{d.nombre}</p>
              <span className="flex items-center gap-1 rounded-lg border border-charcoal/15 px-2.5 py-1 text-xs text-charcoal/70">
